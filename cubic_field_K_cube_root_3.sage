### This file is loaded by 'BCG_cubic.sage'. It contains the data for the complex cubic field Q(beta) with beta^3=3. It loads the following GLOBAL VARIABLES:
...
K_basis_SAGE = K.integral_basis() # This is the basis that SAGE gives: it is [**,**,**].
...
#aa = K.ideal(2).factor()[1][0] # aa is the smoothing ideal
aa = K.ideal(5).factor()[1][0] # aa is the smoothing ideal
#bb = K.ideal(1) # bb is just the ring of integers
bb = K.ideal(2).factor()[1][0] # bb generates the ray class group
q_ff = 3 # Recall that q is the smallest positive integer in ff, which in this case is 3.
N_aa = aa.norm() # We use N_aa to denote the norm of aa, which in this case is 2 (or 5 for the other choice).
...
vareps = y^2-2 # This is the fundamental unit. Note that vareps-1 belongs to ff
varepsRR = yRR^2-2 # The image of the fundamental unit in RR. Note in lands in (0,1).
varepsCC = yCC^2-2
...
# IMPORTANT NOTE: the convention in this file is that vectors representing elements of K are ROW VECTORS. That is: (1,0,0) represents **, (0,1,0) represents ** and (0,0,1) represents **. This means that the map x mapsto vareps x (i.e. multiplication by vareps) corresponds to the map v mapsto v * Eps.

print("Loaded 'cubic_field_K_cube_root_3.sage'. Contains the following variables: ")
print()
print('K:', K)
...
print('The narrow ray class group of conductor ff has order 2.'  )
print()
print('aa = ', aa )
print('N_aa = norm of aa = ' , aa.norm())
print('aa_comp = N_aa * aa^(-1)')
print('bb = ', bb , ' of norm ' , bb.norm() )
#print '(bb has order 1 in the narrow ray class group modulo ff.)' 
